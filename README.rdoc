...
* Database creation
Uses sqlite3 ad default test db defined in database.yml - test.sqlite3
...
Please note that h1_data, h2_data, h3_data and links are stored in db as JSON strings. This will need to be parsed again like this before accessing the array elements:
...
2. Use json_matchers or json_spec gems to match expected JSON with JSON returned by endpoints. See how to represent an array inside a hash (as required for h1_data).
3. For performance improvement:
   - Validate URL passed to index_page_content method in pages_controller in a before_filter method. Return status 400 if validation fails
